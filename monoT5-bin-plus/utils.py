...
class MyModel(pl.LightningModule):
    ...
    def on_train_batch_end(self,trainer,*args, **kwargs):
        ...
        if self.mystep % 500 == 0 and self.mystep >0 :
            ...
            directory = 'monoT5-bin-plus/chk/model-' + str(self.mystep)
            ...
            torch.save(model_to_save.state_dict(), 'monoT5-bin-plus/chk/model-' + str(self.mystep) + '/pytorch_model.bin')
            model_to_save.config.to_json_file('monoT5-bin-plus/chk/model-' + str(self.mystep) + '/config.json')
            self.tokenizer.save_pretrained('monoT5-bin-plus/chk/model-' + str(self.mystep) + '/')    
    ...
    def save_model(self):
        ...
        model_to_save = self.model.module if hasattr(self.model, 'module') else self.model
        torch.save(model_to_save.state_dict(), 'monoT5-bin-plus/chk/model/pytorch_model.bin')
        model_to_save.config.to_json_file('monoT5-bin-plus/chk/model/config.json')
        self.tokenizer.save_pretrained('monoT5-bin-plus/chk/model/')
...
class MyUtils():
    ...
    def gen_prompt(self, query, doc):
        prompt_txt = 'Is the question: \"' + query + "\" answered by the document: \"" + doc + "\"? Give an explanation."
